...
int prepare_send_window(FILE *fp) {
    ...
    if (send_window.left == send_window.right) {
        // step.0 初始化发送窗口[不在todo之内]
        int i;
        for(i = 1;i <= RDT_SENDWIN_LEN; i++){
            if(feof(fp)) break;

            char databuf[RDT_DATA_LEN];
            int datalen = 0;
            datalen = fread(databuf,sizeof(char),RDT_DATA_LEN,fp);
            send_window.rdt_pkts[i%10].pkt_seq = RDT_BEGIN_SEQ + i - 1;
            gettimeofday(&send_window.rdt_pkts[i%10].send_time,0);
            send_window.rdt_pkts[i%10].state = RDT_PKT_ST_SENT;
            send_window.rdt_pkts[i%10].pkt_len = 
                pack_rdt_pkt(databuf, send_window.rdt_pkts[i%10].rdt_pkt,datalen, send_window.rdt_pkts[i%10].pkt_seq, RDT_CTRL_DATA);
    ...
    while(ptr_pkt_left->state == RDT_PKT_ST_ACKED){
        send_window.left++;
        if(feof(fp)) break;

        gettimeofday(&ptr_pkt_left->send_time, 0);
        ptr_pkt_left->state = RDT_PKT_ST_INIT;
        ptr_pkt_left->pkt_seq += RDT_SENDWIN_LEN;
        char databuf[RDT_DATA_LEN];
        int datalen;
        datalen = fread(databuf,sizeof(char),RDT_DATA_LEN,fp);
        ptr_pkt_left->pkt_len = pack_rdt_pkt(databuf, ptr_pkt_left->rdt_pkt, datalen, ptr_pkt_left->pkt_seq, RDT_CTRL_DATA);
        
        pkt_to_send++;
        send_window.right++;
        ptr_pkt_left = &send_window.rdt_pkts[send_window.left % send_window.len];
    }
